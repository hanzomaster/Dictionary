...
public class TranslateApi {
  ...
  /**
   * Call the google translate api from rakuten rapidapi (limit at 50
   * requests/day). Link:
  ...
  private static HttpResponse<JsonNode> translateApi(final String text) throws UnirestException {
    HttpResponse<JsonNode> response = Unirest.post("https://google-translate1.p.rapidapi.com/language/translate/v2")
        .header("content-type", "application/x-www-form-urlencoded").header("accept-encoding", "application/gzip")
        .header("x-rapidapi-key", "1a4e2eacfdmsh859ddddca069bfep129c1djsnd50ac919eb81")
        .header("x-rapidapi-host", "google-translate1.p.rapidapi.com").body("q=" + text + "&target=vi&source=en")
        .asJson();
    return response;
  }
  ...
  static String getTranslatedText(final String text) throws UnirestException {
    ...
    final String translatedText = responseBody.getJSONObject("data").getJSONArray("translations").getJSONObject(0)
